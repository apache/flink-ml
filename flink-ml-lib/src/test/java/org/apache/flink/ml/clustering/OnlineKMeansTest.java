...
public class OnlineKMeansTest extends TestLogger {
    ...
    public void testDecayFactor() throws Exception {
        ...
        assertArrayEquals(
                expectedModelData.weights.getValues(), actualModelData.weights.getValues(), 1e-5);
        ...
        for (int i = 0; i < expectedModelData.centroids.length; i++) {
            assertArrayEquals(
                    expectedModelData.centroids[i].getValues(),
                    actualModelData.centroids[i].getValues(),
                    1e-5);
        }
    }

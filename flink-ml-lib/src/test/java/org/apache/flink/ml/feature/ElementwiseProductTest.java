        ...
        for (Row result : results) {
            if (result.getField(0) == (Object) 0) {
                if (isSparse) {
                    ...
                    assertArrayEquals(
                            EXPECTED_OUTPUT_SPARSE_VEC_INDICES_1, sparseVector.getIndices());
                    assertArrayEquals(
                            EXPECTED_OUTPUT_SPARSE_VEC_VALUES_1, sparseVector.getValues(), 1.0e-5);
                } else {
                    assertArrayEquals(
                            EXPECTED_OUTPUT_DENSE_VEC_ARRAY_1,
                            ((DenseIntDoubleVector) result.getField(outputCol)).getValues(),
                            1.0e-5);
                }
                ...
                if (isSparse) {
                    ...
                    assertArrayEquals(
                            EXPECTED_OUTPUT_SPARSE_VEC_INDICES_2, sparseVector.getIndices());
                    assertArrayEquals(
                            EXPECTED_OUTPUT_SPARSE_VEC_VALUES_2, sparseVector.getValues(), 1.0e-5);
                } else {
                    assertArrayEquals(

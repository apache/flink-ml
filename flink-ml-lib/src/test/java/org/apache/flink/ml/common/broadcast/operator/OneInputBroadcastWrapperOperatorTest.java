        ...
        try (StreamTaskMailboxTestHarness<Integer> harness =
                new StreamTaskMailboxTestHarnessBuilder<>(
                        ...
                        .setupOutputForSingletonOperatorChain(
                                SimpleOperatorFactory.of(oneInputStreamOperator), operatorId)
                        .build()) {
            harness.processElement(new StreamRecord<>(5, 2));
            harness.processElement(new StreamRecord<>(6, 3));

            // Check the output
            assertEquals(
                    Arrays.asList(new StreamRecord<>(5, 2), new StreamRecord<>(6, 3)),

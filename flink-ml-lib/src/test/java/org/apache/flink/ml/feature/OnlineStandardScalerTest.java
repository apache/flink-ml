...
public class OnlineStandardScalerTest extends AbstractTestBase {
    ...
    private static void verifyModelData(
        ...
        assertArrayEquals(expected.mean.getValues(), actual.mean.getValues(), TOLERANCE);
        assertArrayEquals(expected.std.getValues(), actual.std.getValues(), TOLERANCE);
    ...
    private void verifyPredictionResult(
        ...
        for (Row r : collectedResult) {
            ...
            predictions.add((DenseIntDoubleVector) (vec.toDense()));
        }
        ...
        for (int i = 0; i < predictions.size(); i++) {
            assertArrayEquals(
                    expectedOutput.get(i).getValues(), predictions.get(i).getValues(), TOLERANCE);
        }
    }
}

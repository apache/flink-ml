...
public class OnlineLogisticRegression
    ...
    private static class UpdateModel extends AbstractStreamOperator<DenseIntDoubleVector>
        ...
        public void processElement(StreamRecord<DenseIntDoubleVector[]> streamRecord)
            ...
            double[] coefficient = gradientInfo[2].getValues();
            double[] g = gradientInfo[0].getValues();
            for (int i = 0; i < g.length; ++i) {
                if (gradientInfo[1].get(i) != 0.0) {
                    g[i] = g[i] / gradientInfo[1].get(i);
                }
            }
            if (zParam == null) {
            ...
            output.collect(new StreamRecord<>(Vectors.dense(coefficient)));
        }
    }
            ...
            for (Row point : points) {
                ...
                if (vec instanceof DenseIntDoubleVector) {
                    ...
                    for (int i = 0; i < modelData.size(); ++i) {
                        gradient[i] += (p - label) * dvec.get(i);
                        weightSum[i] += 1.0;
                    }
                } else {
                    SparseIntDoubleVector svec = (SparseIntDoubleVector) vec;
                    int[] indices = svec.getIndices();
                    double[] values = svec.getValues();

...
public class BroadcastUtils {
    ...
    private static <OUT> DataStream<OUT> cacheBroadcastVariables(
        ...
        MultipleInputTransformation<OUT> transformation =
                new MultipleInputTransformation<OUT>(
                        "broadcastInputs",
                        new CacheStreamOperatorFactory <OUT>(
    ...
    private static <OUT> DataStream<OUT> buildGraph(
        ...
        DraftExecutionEnvironment draftEnv = new DraftExecutionEnvironment(env);
        ...
        for (int i = 0; i < inputList.size(); i++) {
            ...
            draftSources.add(draftEnv.addDraftSource(inputList.get(i), type));
            inTypes[i] = type;
        }
        ...
        draftEnv.copyToActualEnvironment(
                new BroadcastWrapper<>(broadcastStreamNames, inTypes, isBlocking));
        DataStream<OUT> outStream = draftEnv.getActualStream(draftOutStream.getId());
        return outStream;
    }

    /**
     * Support withBroadcastStream in DataStream API. Broadcast data streams are available at all parallel instances of
     * the input operators. A broadcast data stream is registered under a certain name and can be retrieved under that
     * name via {@link BroadcastContext}.getBroadcastVariable(...).
     *
     * In detail, the broadcast input data streams will be consumed first and cached as static variables in
     * {@link BroadcastContext}.
     * For now the non-broadcast input are blocking and cached to avoid the possible deadlocks.
     *
     ...
     * @param userDefinedFunction the user defined logic in which users can access the broadcast data streams and
     *                            produce the output data stream.
     * @param outType type of the output data stream.
